...
router.get(
  '/',
  isAuthorized(Permissions.Schedule.Read),
  asyncHandler(async (req, res) => {
    ...
    res
      .status(200)
      .json(await scheduleService.getSchedules({ userId, fromDate, toDate }));
  })
);

router.post(
  '/',
  isAuthorized(Permissions.Schedule.CRUD),
  asyncHandler(async (req, res) => {
    ...
    res
      .status(201)
      .json(
        await scheduleService.createSchedule({ workDate, userId, shiftLength })
      );
  })
);
